...
int dftDemo() {
	...
	cv::Mat srcMat = imread("D:/Project_data/C++/Project_12_2/testImages/rose.jpg", 0);
	cv::Mat magMat;
...
int removeFrequnce()
{
	...
	selectPolygon(magMat, maskMat, maskMat2);
...
{
	cv::Mat dst;
	cv::Mat src = imread("D:/Project_data/C++/Project_12_2/testImages/rose.jpg", 0);
...
int mouseROI()
{
	cv::Mat srcMat = imread("D:/Project_data/C++/Project_12_2/testImages/rose.jpg");
	cv::Mat dstMat,dstMat2;

	selectPolygon(srcMat, dstMat, dstMat2);
...
int ifftDemo()
{
	...
	cv::Mat src = imread("特斯拉.jpg", 0);
	cv::Mat src2 = imread("牛顿.jpg", 0);
	int m = getOptimalDFTSize(src.rows); //2,3,5的倍数有更高效率的傅里叶变换
	int n = getOptimalDFTSize(src.cols);
	Mat padded, padded2;
	//把灰度图像放在左上角,在右边和下边扩展图像,扩展部分填充为0;
	...
	magnitude(planes[0], planes[1], planes_true);//幅度谱mag
	phase(planes[0], planes[1], ph);//相位谱ph
	magnitude(planes2[0], planes2[1], planes_true2);//幅度谱mag
	phase(planes2[0], planes2[1], ph2);//相位谱ph
	Mat A = planes[0];
	Mat B = planes[1];
	...
	cv::Mat dstMat;
	addWeighted(dspMat, 0.6, dspMat2, 0.4, 0, dstMat);
	imshow("dst", dspMat);
	imshow("dst2", dspMat2);
